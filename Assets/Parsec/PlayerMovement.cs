...
class PlayerMovement : MonoBehaviour
{
    ...
    public int player = 1;
    private Rewired.Player m_re_player;

    public void initReInput(CustomController controller, CustomController keyboard, CustomController mouse)
    {
        m_re_player = ReInput.players.GetPlayer(player);
        if (player == 0)
        {
            m_re_player.controllers.AddController(ReInput.controllers.Joysticks[0], true);
